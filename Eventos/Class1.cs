...
public partial class Plugintest
{
    ...
    public static unsafe CBaseProp? TraceObject(CounterStrikeSharp.API.Modules.Utils.Vector _origin, CounterStrikeSharp.API.Modules.Utils.QAngle _viewangles, bool drawResult = false, bool fromPlayer = false)
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            TraceFunc = NativeAPI.FindSignature(Addresses.ServerPath, "48 B8 ? ? ? ? ? ? ? ? 55 48 89 E5 41 57 41 56 49 89 D6 41 55");
        ...
        if (result)
        {
            ...
            if (_trace->HitEntity != null)
            {
                Console.WriteLine("Entidad golpeada no nula");
                var hitEntity = new CBaseEntity((nint)_trace->HitEntity);
                if (hitEntity != null && hitEntity.DesignerName.Contains("prop"))
                {
                    Console.WriteLine("Entidad agarrada con exito");
                    return new CBaseProp(hitEntity.Handle);
                }
